...
export class IdentifyPhase {
  ...
  private async handleMachineId(id: string): Promise<boolean> {
    try {
      ...
      if (machine) {
        ...
        // Check if reference database download is needed
        // The referenceDbUrl is stored in machine data (configured by operator)
      ...
      // Machine not found - auto-create WITHOUT referenceDbUrl
      // The operator must configure the referenceDbUrl via MachineSetupForm
      logger.info(`🆕 Auto-creating machine ${id} (no reference DB URL yet)`);
   ...
   *
   * Note: The NFC link contains ONLY the machine ID.
   * The database URL is stored in the machine data (referenceDbUrl field),
   * NOT in the NFC link. This allows updating the database without rewriting NFC tags.
   */
  private async handleDeepLink(): Promise<void> {
    ...
    if (hash && hash.startsWith('#/m/')) {
      ...
      if (match.type === 'machine' && match.machineId) {
        ...
        isHashRoute = true;
        logger.info(`🔗 Deep link parsed: machineId=${machineId}`);
      }
    }
    ...
    try {
      // Note: referenceDbUrl is read from machine data, not from the deep link
  ...
  private async handleNfcWrite(): Promise<void> {
    ...
    // Note: The NFC link contains ONLY the machine ID.
    // The database URL is stored in machine data, not in the NFC link.
